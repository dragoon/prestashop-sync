...
def display_blog_page(request, tag=None, language=None, username=None, year=None, month=None, page=1):
    """
    ...
    if tag:
        ...
        articles = Article.objects.live(user=request.user).select_related().filter(language=language)
        template = 'articles/by_language.html'
        context['language'] = language
    else:
        # listing articles with no particular filtering
        language = get_language()
        articles = Article.objects.live(user=request.user).filter(language=language)
...
def display_article(request, year, language, slug, template='articles/article_detail.html'):
    """Displays a single article."""
    try:
        article = Article.objects.live(user=request.user).get(publish_date__year=year, language=language, slug=slug)
