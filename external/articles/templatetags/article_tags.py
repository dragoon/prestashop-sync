...
class GetArticleArchivesNode(template.Node):
    ...
    def render(self, context):
        ...
        cache_lang_key = 'articles_lang'
        language = get_language()
        dt_archives = cache.get(cache_key)
        if dt_archives is None or (language!=cache.get(cache_lang_key)):
            archives = {}
            user = context.get('user', None)

            # iterate over all live articles
            for article in Article.objects.live(user=user).filter(language=language).select_related():
            ...
            cache.set(cache_lang_key, language)

        # put our collection into the context
        context[self.varname] = dt_archives
        return ''

class GetArticleLanguagesNode(template.Node):
    """
    Retrieves a list of languages in which articles have been posted.
    """
    def __init__(self, varname):
        self.varname = varname
    ...
    def render(self, context):
        ...
        for language in settings.LANGUAGES:
            if Article.objects.live(user=user).select_related().filter(language=language[0]).count() > 0:
                dt_languages.append(language[0])

        context[self.varname] = dt_languages
        return ''
...
def get_article_languages(parser, token):
    """
    Retrieve a list of languages used by posted articles
    """
    args = token.split_contents()
    argc = len(args)
    ...
    try:
        ...
        raise template.TemplateSyntaxError('get_article_languages syntax: {% get_article_languages as varname %}')

    return GetArticleLanguagesNode(args[2])
...
register.tag(get_article_languages)
