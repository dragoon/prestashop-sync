...
class TranslatedDocument(models.Model):
    ...
    def get_absolute_url(self):
        return self.language_url

    def get_translated(self):
        if not self.translation:
            return self
        else:
            return self.__class__.objects.get(id=self.translation.id)
    ...
    def get_translations(self):
        translated = self.get_translated()
        translations = translated.translation_set.all()
        return list(translations) + [translated]

    def get_translations_menu(self):
        objs = dict((obj.language, obj) for obj in self.get_translations())
        menu = []
        for language, title in settings.LANGUAGES:
            entry = dict(language=language, title=_(title))
            if language in objs:
                entry['object'] = objs[language]
            menu.append(entry)
        return menu


class Article(TranslatedDocument):
    ...
    @models.permalink
    def get_absolute_url(self):
        return ('articles_display_article', (int(self.publish_date.year), self.language, self.slug,))
    ...
    def get_previous_article(self):
        ...
        if not self._previous:
            try:
                qs = Article.objects.live().exclude(id__exact=self.id)
                article = qs.filter(language=self.language, publish_date__lte=self.publish_date).order_by('-publish_date')[0]
            except (Article.DoesNotExist, IndexError):
                article = None
